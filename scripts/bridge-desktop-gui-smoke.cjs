...
assert.match(source, /ToolStripMenuItem\('Changelog\.\.\.'\)/)
assert.match(source, /function Get-ChangelogSections/)
assert.match(source, /function Show-ChangelogWindow/)
assert.match(source, /function Start-Changelog/)
assert.match(source, /function Complete-ChangelogRequest/)
assert.match(source, /'-ReleaseTag', "v\$CurrentVersion"/)
assert.match(source, /Show-ChangelogWindow -LatestResult \$script:LatestReleaseResult -InstalledResult \$script:InstalledReleaseResult/)
const updateMenuAdd = source.indexOf('[void]$helpMenu.DropDownItems.Add($checkUpdatesMenuItem)')
const changelogMenuAdd = source.indexOf('[void]$helpMenu.DropDownItems.Add($changelogMenuItem)')
const helpSeparatorAdd = source.indexOf('[void]$helpMenu.DropDownItems.Add((New-Object System.Windows.Forms.ToolStripSeparator))', changelogMenuAdd)
assert(updateMenuAdd >= 0 && changelogMenuAdd > updateMenuAdd && helpSeparatorAdd > changelogMenuAdd, 'Help > Changelog must sit directly below Check for updates')
