...
withQuietRelayLogs(() => {
  ...
  assert.strictEqual(sentDownstream.length, 2)
  ...
  assert.strictEqual(sentDownstream[1].name, 'update_block')
  assert.deepStrictEqual(sentDownstream[1].packet.position, { x: 83, y: 64, z: 669 })
  assert.strictEqual(sentDownstream[1].packet.block_runtime_id, craftingTable.block_runtime_id)
  assert.deepStrictEqual(sentDownstream[1].packet.flags, { neighbors: true, network: true })
  assert.strictEqual(sentDownstream[1].packet.layer, 0)
