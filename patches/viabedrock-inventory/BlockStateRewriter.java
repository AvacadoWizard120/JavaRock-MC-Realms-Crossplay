...
public class BlockStateRewriter implements StorableObject {
    ...
    public BlockStateRewriter(final BlockProperties[] blockProperties, final boolean hashedRuntimeBlockIds) {
        ...
        final List<String> missingMappingSamples = new ArrayList<>(MISSING_MAPPING_SAMPLE_LIMIT);
        ...
        if (missingMappingCount > 0) {
            ViaBedrock.getPlatform().getLogger().log(Level.WARNING,
                    "Missing " + missingMappingCount + " bedrock -> java block state mapping(s); applied the INFO_UPDATE fallback for each. "
                            + "Sample Bedrock runtime IDs/states (first " + missingMappingSamples.size() + "): " + missingMappingSamples);
        }
