...
public class EntityPackets {
    ...
    public static void register(final BedrockProtocol protocol) {
        protocol.registerClientbound(ClientboundBedrockPackets.ADD_ENTITY, ClientboundPackets26_1.ADD_ENTITY, wrapper -> {
            ...
            final Entity entity;
            final EntityTypes26_2 javaEntityType = BedrockProtocol.MAPPINGS.getBedrockToJavaEntities().get(type);
            if (javaEntityType != null) {
                ...
                if (entityDefinition != null) {
                    if (resourcePackStorage.isLoadedOnJavaClient()) {
                        ...
                        entity = entityTracker.addEntity(entityUniqueId, entityRuntimeId, type, EntityTypes26_2.PIG);
                    }
                } else {
            ...
            wrapper.write(Types.VAR_INT, javaAddEntityData(wrapper, entity, entityData)); // data
            ...
            entity.updateEntityData(javaPostSpawnEntityData(entity, entityData));
        });
        ...
        protocol.registerClientbound(ClientboundBedrockPackets.ADD_PAINTING, ClientboundPackets26_1.ADD_ENTITY, wrapper -> {
            ...
            final Entity entity = entityTracker.addEntity(entityUniqueId, entityRuntimeId, "minecraft:painting", EntityTypes26_2.PAINTING);
            entity.setPosition(position);
        ...
        protocol.registerClientbound(ClientboundBedrockPackets.TAKE_ITEM_ENTITY, ClientboundPackets26_1.TAKE_ITEM_ENTITY, wrapper -> {
            ...
            if (itemEntity == null || collectorEntity == null || itemEntity.javaType() != EntityTypes26_2.ITEM) {
                wrapper.cancel();
                return;
            }
    ...
    private static int javaAddEntityData(final PacketWrapper wrapper, final Entity entity, final EntityData[] entityData) {
        if (!entity.javaType().is(EntityTypes26_2.FALLING_BLOCK)) {
            return 0;
        }

        for (EntityData data : entityData) {
            if (data.id() != ActorDataIDs.VARIANT.getValue() || !(data.getValue() instanceof Number blockRuntimeId)) {
                continue;
            }

            final int javaBlockState = wrapper.user().get(BlockStateRewriter.class).javaId(blockRuntimeId.intValue());
            if (javaBlockState != -1) {
                return javaBlockState;
            }
            ViaBedrock.getPlatform().getLogger().log(Level.WARNING, "Missing Java block state for falling block runtime id: " + blockRuntimeId);
            return 0;
        }

        ViaBedrock.getPlatform().getLogger().log(Level.WARNING, "Falling block entity is missing Bedrock VARIANT metadata");
        return 0;
    }

    private static EntityData[] javaPostSpawnEntityData(final Entity entity, final EntityData[] entityData) {
        if (!entity.javaType().is(EntityTypes26_2.FALLING_BLOCK)) {
            return entityData;
        }

        final List<EntityData> remainingEntityData = new ArrayList<>(entityData.length);
        for (EntityData data : entityData) {
            if (data.id() != ActorDataIDs.VARIANT.getValue()) {
                remainingEntityData.add(data);
            }
        }
        return remainingEntityData.toArray(EntityData[]::new);
    }
