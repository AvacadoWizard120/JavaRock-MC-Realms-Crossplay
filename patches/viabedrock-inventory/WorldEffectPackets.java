...
public class WorldEffectPackets {
    ...
    public static void register(final BedrockProtocol protocol) {
        ...
        protocol.registerClientbound(ClientboundBedrockPackets.BLOCK_EVENT, ClientboundPackets26_1.BLOCK_EVENT, wrapper -> {
            ...
            BlockPosition pairedChestPosition = null;
            if (tag == null) {
            ...
            switch (tag) {
                ...
                case CustomBlockTags.ENDER_CHEST, CustomBlockTags.SHULKER_BOX -> {
                    if (type == 1) { // open / close
                        wrapper.write(Types.UNSIGNED_BYTE, (short) type); // event type
                        wrapper.write(Types.UNSIGNED_BYTE, (short) MathUtil.clamp(data, 0, 255)); // event data
                    } else {
            ...
            final int javaBlock = BedrockProtocol.MAPPINGS.getJavaBlocks().get(BedrockProtocol.MAPPINGS.getJavaBlockStates().inverse().get(blockStateRewriter.javaId(blockState)).namespacedIdentifier());
            wrapper.write(Types.VAR_INT, javaBlock); // block
            if (pairedChestPosition != null) {
                sendPairedChestBlockEvent(wrapper.user(), pairedChestPosition, type, data, javaBlock);
            }
        });
    }

    private static void sendPairedChestBlockEvent(final UserConnection user, final BlockPosition position, final int type, final int data, final int javaBlock) {
        final PacketWrapper paired = PacketWrapper.create(ClientboundPackets26_1.BLOCK_EVENT, user);
        paired.write(Types.BLOCK_POSITION1_14, position);
        paired.write(Types.UNSIGNED_BYTE, (short) type);
        paired.write(Types.UNSIGNED_BYTE, (short) MathUtil.clamp(data, 0, 255));
        paired.write(Types.VAR_INT, javaBlock);
