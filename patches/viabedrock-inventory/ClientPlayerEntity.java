...
public class ClientPlayerEntity extends PlayerEntity {
    ...
    public void tick() {
        ...
        final long oldestRewindTick = (long) this.age() - Math.max(32, this.gameSession.getMovementRewindHistorySize() + 4);
        this.movementPositionHistory.headMap(oldestRewindTick, false).clear();
    ...
    public Position3f rebaseMovementCorrection(final Position3f correctedPosition, final long tick) {
        final Position3f predictedPosition = this.movementPositionHistory.get(tick);
        if (predictedPosition == null) return correctedPosition;

        final float dx = correctedPosition.x() - predictedPosition.x();
        final float dy = correctedPosition.y() - predictedPosition.y();
        final float dz = correctedPosition.z() - predictedPosition.z();
        for (Map.Entry<Long, Position3f> entry : this.movementPositionHistory.tailMap(tick, true).entrySet()) {
            final Position3f position = entry.getValue();
            entry.setValue(new Position3f(position.x() + dx, position.y() + dy, position.z() + dz));
        }
